...
from .utils_motifs import obj_edge_vectors, to_onehot, nms_overlaps, encode_box_info, rel_vectors
...
class SemanticContextModule(nn.Module):
    def __init__(self, cfg, in_channels, obj_classes, rel_classes):
        super(SemanticContextModule, self).__init__()
        self.cfg = cfg
        self.obj_classes = obj_classes
        self.rel_classes = rel_classes
        ...
        obj_embed_vecs = obj_edge_vectors(self.obj_classes, wv_dir=self.cfg.GLOVE_DIR, wv_dim=self.obj_embed_dim)
        rel_embed_vecs = rel_vectors(self.rel_classes, wv_dir=self.cfg.GLOVE_DIR, wv_dim=self.rel_embed_dim)

        self.num_obj_cls = len(obj_classes)  # 151
        self.num_rel_cls = len(rel_classes)  # 51

        self.obj_embed = nn.Embedding(self.num_obj_cls, self.obj_embed_dim)  # 151 -> 200
        self.rel_embed = nn.Embedding(self.num_rel_cls, self.rel_embed_dim)  # 51 -> 200
        with torch.no_grad():
            self.obj_embed.weight.copy_(obj_embed_vecs, non_blocking=True)
            self.rel_embed.weight.copy_(rel_embed_vecs, non_blocking=True)

        self.lin_tri = nn.Linear(2 * self.obj_embed_dim + self.rel_embed_dim, self.hidden_dim)  # 600 -> 512

        self.D_Encoder = TransformerEncoder(self.Discriminator_layer, self.num_head, self.k_dim,
                                            self.v_dim, self.hidden_dim, self.inner_dim, self.dropout_rate)

    def forward(self, obj_logits, rel_logits, rel_pair_idxs, proposals, rel_labels, groundTruth=False):

        num_objs = [len(p) for p in proposals]
        num_rels = [r.shape[0] for r in rel_pair_idxs]

        feat_dim = self.obj_embed_dim * 2 + self.rel_embed_dim  # 600
        if groundTruth:
            obj_labels = cat([proposal.get_field("labels") for proposal in proposals], dim=0).long()
            rel_labels = cat(rel_labels, dim=0).long()
            obj_embed_feat = self.obj_embed(obj_labels)  # [1280, 200]
            rel_embed_feat = self.rel_embed(rel_labels)  # [3000+, 200]
        else:
            use_gt_label = self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL
            obj_labels = cat([proposal.get_field("labels") for proposal in proposals],
                             dim=0) if use_gt_label else None
            if obj_labels is not None:
                obj_labels = obj_labels.long()
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL:
                ...
                obj_logits = cat(obj_logits, dim=0).clamp(min=1e-5)  # [1280, 151]
                obj_prop = F.softmax(obj_logits, dim=1)
                obj_mask = 1 - (obj_prop < 1e-3).long()
                obj_embed_feat = (obj_prop * obj_mask) @ self.obj_embed.weight  # [1280, 200]

            rel_logits = cat(rel_logits, dim=0).clamp(min=1e-5)  # [3000+, 51]
            rel_prop = F.softmax(rel_logits, dim=1)
            rel_mask = 1 - (rel_prop < 1e-3).long()
            rel_embed_feat = (rel_prop * rel_mask) @ self.rel_embed.weight  # [3000+, 200]

        obj_embed_feat = obj_embed_feat.split(num_objs)
        rel_embed_feat = rel_embed_feat.split(num_rels)
        triple_feat = []
        new_num_rels = []
        for pair_idx, obj_feat, rel_feat in zip(rel_pair_idxs, obj_embed_feat, rel_embed_feat):
            numb = pair_idx.size(0)
            if numb == 0 or rel_feat.size(0) == 0:
                continue
            new_num_rels.append(numb + 1)  
            feat = torch.zeros(numb + 1, feat_dim).cuda()
            feat[1:] = torch.cat((obj_feat[pair_idx[:, 0]], rel_feat, obj_feat[pair_idx[:, 1]]), dim=-1)
            feat[0] = feat[1:].mean(dim=0)  
            triple_feat.append(feat)

        triple_feat = cat(triple_feat, dim=0)
        triple_rep = self.lin_tri(triple_feat)  
        return self.D_Encoder(triple_rep, new_num_rels), new_num_rels, 