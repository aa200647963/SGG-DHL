...
from .model_transformer import TransformerContext, SemanticContextModule
...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.semantic_context_module = SemanticContextModule(config, in_channels, obj_classes, rel_classes)
        self.semantic_compress = nn.Linear(self.hidden_dim, self.num_rel_cls)
        layer_init(self.semantic_compress, xavier=True)

        # Coarse-grained Learning Branch Classifier
        self.post_cat_2 = nn.Linear(self.hidden_dim * 2, self.pooling_dim)  # 1024 -> 4096
        self.rel_compress_2 = nn.Linear(self.pooling_dim, self.num_rel_cls)  # 4096 -> 51
        self.ctx_compress_2 = nn.Linear(self.hidden_dim * 2, self.num_rel_cls)  # 1024 -> 51
        layer_init(self.rel_compress_2, xavier=True)
        layer_init(self.ctx_compress_2, xavier=True)
        layer_init(self.post_cat_2, xavier=True)
        self.iter = 0 
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # Coarse-grained Learning Branch Classify
        if self.training:
            self.iter += 1 
        ...
        if self.training and self.iter > 10000: 
            res_true, new_num_rels = self.semantic_context_module(obj_dists, rel_dists,
                                                                             rel_pair_idxs,
                                                                             proposals, rel_labels,
                                                                             True)  
            res_false, new_num_rels = self.semantic_context_module(obj_dists, rel_dists,
                                                                               rel_pair_idxs,
                                                                               proposals, rel_labels,
                                                                               False)  
            res_true = res_true.detach()  
            res_true_rep = res_true.split(new_num_rels)  
            res_false_rep = res_false.split(new_num_rels)
            global_true_rep = []
            global_false_rep = []

            semantic_rel_rep = []  
            for t, f in zip(res_true_rep, res_false_rep):
                global_true_rep.append(t[0])
                global_false_rep.append(f[0])
                semantic_rel_rep.append(f[1:])

            semantic_loss = self._computer_mse_loss(global_true_rep, global_false_rep)
            add_losses.update({'semantic_loss': semantic_loss})  

            modify_dists = self.semantic_compress(cat(semantic_rel_rep, dim=0))
            final_dists = cat(rel_dists, dim=0) + modify_dists

            # Knowledge distillation
            kd_loss = self.kd_ce_loss(final_dists, rel_dists_2, rel_labels, temperature=2)
            add_losses.update({"kd_loss": kd_loss*0.05})  

            final_dists = final_dists.split(num_rels, dim=0)
            ...
            res_false, new_num_rels = self.semantic_context_module(obj_dists, rel_dists,
                                                                               ...
                                                                               False)
            res_false_rep = res_false.split(new_num_rels)
            semantic_rel_rep = [f[1:] for f in res_false_rep]
        ...
        if self.attribute_on:
            ...
            if self.iter <= 10000:
                final_dists = rel_dists
            return obj_dists, final_dists, add_losses, add_data

    
    def _computer_mse_loss(self, true_rep, false_rep):
        true_rep = cat(true_rep, dim=0)
        false_rep = cat(false_rep, dim=0)
        return torch.nn.functional.mse_loss(false_rep, true_rep)
    
    def kd_ce_loss(self, logits_S, logits_T, rel_labels, temperature=1):
        '''
        ...
        the_head_idx = self.head_pred_idx.clone()
        rel_labels = cat(rel_labels, dim=0)  # [2408]
        num_head = the_head_idx.size(0)  # 16 
        
        rel_labels = rel_labels[None,:].expand(num_head, -1)  # [16, 2408]
        the_head_idx = the_head_idx[:,None]  # [16,1]
        mask = (rel_labels == the_head_idx).sum(0).bool()  # 
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.ctx_compress = nn.Linear(self.hidden_dim * 2, self.num_rel_cls)  # 1024 -> 51
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        for pair_idx, head_rep, tail_rep, obj_pred in zip(rel_pair_idxs, head_reps, tail_reps, obj_preds):
            ...
            pair_preds.append( torch.stack((obj_pred[pair_idx[:,0]], obj_pred[pair_idx[:,1]]), dim=1) )
        prod_rep = cat(prod_reps, dim=0)
        pair_pred = cat(pair_preds, dim=0).long()
...
class VCTreePredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls)  # 4096 -> 51
        self.ctx_compress = nn.Linear(self.hidden_dim * 2, self.num_rel_cls)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        ctx_gate = self.post_cat(prod_rep)  # 1024 -> 4096 
